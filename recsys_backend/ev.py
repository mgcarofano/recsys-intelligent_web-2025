...
# ========================================
class RatingPredictorRecommender:
    """
    ...
    def predict(self, user, item):
        ...
        # LOGICA BASE da build_ratings_complemented.py
        ...
        # AGGIUNTA BIAS TERMS (VERSIONE CORRETTA)
        if self.use_bias and self.global_mean is not None:
            # Ottieni i bias
            i_bias = self.item_bias.get(mid, 0.0)

            # APPROCCIO 1: Aggiungi solo l'item bias (più conservativo)
            # La predizione base già include implicitamente il comportamento dell'utente
            # Aggiungiamo solo info sul film che il content-based non può catturare bene

            # Peso adattivo basato sulla confidenza della predizione
            # Se la similarità è alta, ci fidiamo di più del content-based
            max_sim = sims.max() if len(sims) > 0 else 0.0
            confidence = min(1.0, max_sim * 2.0)  # 0.5+ similarità → alta confidenza

            # Blending: alta confidenza → più peso al content-based
            #           bassa confidenza → più peso al bias
...
def evaluate_predictions(algo_name, algo, train, test):
    ...
    """
    if isinstance(algo, RatingPredictorRecommender):
...
def main():
    ...
    # Crea DUE versioni del custom predictor: con e senza bias
    custom_predictor_no_bias = RatingPredictorRecommender(X, movies_df, use_bias=False)
    custom_predictor_with_bias = RatingPredictorRecommender(X, movies_df, use_bias=True)

    algorithms = {
        'CustomPredictor (no bias)': custom_predictor_no_bias,
        'CustomPredictor (with bias)': custom_predictor_with_bias,
    ...
    # 4. CALCOLO E VISUALIZZAZIONE MAE E RMSE
    print("\n" + "=" * 80)
    print("RISULTATI - METRICHE DI ERRORE")
    print("=" * 80 + "\n")

    # Rimuove le righe dove la predizione è fallita (NaN)
    ...
    if 'CustomPredictor (no bias)' in results.index and 'CustomPredictor (with bias)' in results.index:
        mae_no_bias = results.loc['CustomPredictor (no bias)', 'MAE']
        mae_with_bias = results.loc['CustomPredictor (with bias)', 'MAE']
        improvement = ((mae_no_bias - mae_with_bias) / mae_no_bias) * 100
        print(f"\n📊 Miglioramento con bias: {improvement:.2f}% riduzione MAE")
