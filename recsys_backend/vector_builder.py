...
# Associo ogni file a una categoria
metadata_files = {
...
# === Step 2: Collect all features per category ===
movie_to_features = defaultdict(list)
all_features = []

print("Scanning all metadata files to build feature list (with categories) and mapping...")
    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if movie_id in movie_id_to_index:
                # Aggiungo la coppia (feature, categoria)
                feature_key = (category, value)
                movie_to_features[movie_id].append(feature_key)
                all_features.append(feature_key)

# === Step 3: Build global index for all features (with categories) ===
# Rimuovo duplicati mantenendo le coppie (feature, categoria)
...
print(f"Total unique (category, feature) pairs: {len(unique_features)}")

# === Step 4: Build sparse matrix (movies × features) ===
...
# === Step 5: Save sparse matrix and feature index ===
